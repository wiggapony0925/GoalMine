...
class MockDB:
    ...
    def save_chat_context(self, phone, context): pass
    def load_chat_context(self, phone): return []
...
async def test_full_suite():
    ...
    await run_scenario(handler, test_phone, "Analyze Mexico vs South Africa", "Natural Analysis Request")
    ...
    await run_scenario(handler, test_phone, "I have 50 dollars what should I get on", "Budget Follow-up (Should use Strategic Advisor)")

    # --- SCENARIO 5: PARLAY STRATEGY ---
    await run_scenario(handler, test_phone, "Should I parlay the draw with Mexico win?", "Strategy Query (Parlay)")

    # --- SCENARIO 6: NEW MATCH (Reset Context) ---
    await run_scenario(handler, test_phone, "What about England vs Germany?", "New Match Request (Should detect new teams)")
    ...
    await run_scenario(handler, test_phone, "How do I make sushi?", "Off-topic Query (Gatekeeper Check)")

    # --- SCENARIO 9: HIGH BUDGET ANALYSIS ---
    await run_scenario(handler, test_phone, "Analyze Mexico vs South Africa with $1000 budget and 5 bets", "High-Budget Specific Command")

    # --- SCENARIO 10: PARTIAL NAMES ---
    await run_scenario(handler, test_phone, "Mex vs S. Afr", "Partial Team Names")

    # --- SCENARIO 11: EXTREME BUDGET ---
    await run_scenario(handler, test_phone, "Analyze with $0 budget", "Zero Budget Check")

    # --- SCENARIO 12: BUDGET ONLY FOLLOW-UP ---
    await run_scenario(handler, test_phone, "$250 please", "Budget-only message after match analysis")

    # --- SCENARIO 13: INVALID TEAM ---
    await run_scenario(handler, test_phone, "Analyze Mars vs Jupiter", "Invalid Team Scenario")

    print("\n" + "="*60)
    print("📊 TEST SUMMARY")
    print("="*60)
    print(f"Total Scenarios Run: 9")
