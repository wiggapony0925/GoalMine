...
class TestCoreInfrastructure:
    ...
    async def test_llm_query_basic(self):
        ...
        with patch(
            "core.initializer.llm.client.chat.completions.create"
        ) as mock_create:
            mock_response = Mock()
    ...
    async def test_llm_json_mode(self):
        ...
        with patch(
            ...
            result = await query_llm(
                system_prompt="Return JSON", user_content="Test", json_mode=True
            )
    ...
    async def test_llm_retry_logic(self):
        ...
        with patch(
            ...
            result = await query_llm(system_prompt="Test", user_content="Test")
...
class TestAgentSwarm:
    ...
    async def test_logistics_fatigue_calculation(self):
        ...
        with patch("core.initializer.llm.query_llm") as mock_llm:
            ...
            result = await agent.analyze(
                "MetLife Stadium, East Rutherford", "Estadio Azteca, Mexico City"
            )
    ...
    async def test_narrative_sentiment_analysis(self):
        ...
        with patch("agents.narrative.api.google_news.fetch_headlines") as mock_news:
            ...
            with patch(
                "agents.narrative.api.reddit_api.RedditScanner.scan_team_sentiment"
            ) as mock_reddit:
...
class TestOrchestrator:
    ...
    async def test_orchestrator_cache(self):
        ...
        with patch(
            "agents.logistics.logistics.LogisticsAgent.analyze", new_callable=AsyncMock
        ) as mock:
            mock.return_value = {"branch": "logistics", "fatigue_score": 5}

            await orchestrator.generate_betting_briefing(match_info)
...
class TestBigDaddy:
    ...
    async def test_big_daddy_bet_generation(self):
        ...
        with patch("core.initializer.database.Database.load_memory") as mock_load:
            ...
            with patch("core.initializer.llm.query_llm") as mock_llm:
                ...
                result = await generate_bet_recommendations(
                    num_bets=3, user_phone="1234567890"
                )
    ...
    async def test_big_daddy_uses_correct_prompt(self):
        ...
        with patch(
            "core.initializer.database.Database.load_memory", return_value=mock_god_view
        ):
            with patch("core.initializer.llm.query_llm") as mock_llm:
                ...
                assert "GoalMine Intelligence Chief" in str(
                    call_args
                )  # From the prompt
        ...
        with patch(
            ...
            with patch("core.initializer.llm.query_llm") as mock_llm:
                ...
                result = await generate_strategic_advice(
                    user_phone="test", question="What's the best parlay strategy?"
                )
            ...
            with patch("core.initializer.llm.query_llm") as mock_llm:
                ...
                with patch.object(wa, "send_message"):
                    await handler.handle_incoming_message(
                        "1234567890", "Tell me more about this match"
                    )
...
class TestConfiguration:
    ...
    def test_settings_load(self):
        ...
        assert settings.get("app.interaction_mode") in [
            "CONVERSATIONAL",
            "BUTTON_STRICT",
        ]
