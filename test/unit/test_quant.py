...
def test_quant_engine():
    ...
    print(
        f"   South Africa Win: {best_odds['away']['price']} ({best_odds['away']['book']})"
    )
    ...
    result = run_quant_engine(
        home_xg, away_xg, best_odds, user_budget, "Mexico", "South Africa"
    )
    ...
    if result["top_plays"]:
        for i, play in enumerate(result["top_plays"], 1):
            ...
            print(
                f"      True Prob: {play['true_probability']}% vs Implied: {play['implied_probability']}%"
            )
    ...
    for outcome, key in [
        ("Mexico Win", "home"),
        ("Draw", "draw"),
        ("South Africa Win", "away"),
    ]:
        true_prob = probs[
            "a_win" if key == "home" else ("draw" if key == "draw" else "b_win")
        ]
...
def test_with_different_budgets():
    ...
    for budget in [50, 100, 500, 1000]:
        result = run_quant_engine(
            home_xg, away_xg, best_odds, budget, "Mexico", "South Africa"
        )
        print(f"\n💵 Budget: ${budget}")
        if result["top_plays"]:
            total_stake = sum(p["stake"] for p in result["top_plays"])
            print(
                f"   Total Stake: ${total_stake:.2f} ({total_stake / budget * 100:.1f}% of bankroll)"
            )
...
def test_edge_cases():
    ...
    print(
        f"   Home: {probs['a_win'] * 100:.1f}% | Draw: {probs['draw'] * 100:.1f}% | Away: {probs['b_win'] * 100:.1f}%"
    )
