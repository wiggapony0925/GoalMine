       ...
       webhook_payload = {
        ...
        with app.test_client() as client:
            ...
            with patch('core.initializer.llm.query_llm') as mock_llm, \
                 patch('core.initializer.whatsapp.WhatsAppClient.send_message') as mock_send, \
                 ...
                 patch('agents.logistics.api.open_meteo.fetch_weather_data') as mock_weather, \
                ...
                llm_responses = [
                    ...
                    json.dumps({"fatigue_score": 7, "primary_risk": "Altitude"}),  # Logistics
                    json.dumps({"xg_adjustment_a": 0.3, "xg_adjustment_b": -0.15}),  # Tactics
                    json.dumps({"value_score": "A-", "edge_percentage": 12.3}),  # Market
                    json.dumps({"sentiment_score": 8.5, "morale_impact": "Boost", "narrative_adjustment": 0.15}),  # Narrative Home
                    json.dumps({"sentiment_score": 4.2, "morale_impact": "Drop", "narrative_adjustment": -0.10}),  # Narrative Away
                    """# BET 1
💰 Argentina Win (@ 1.85)
📊 Intelligence:
...
• Narrative: Home morale 8.5/10"""  # Big Daddy
                ...
                mock_news.return_value = [{"title": "Test news", "link": "http://test.com"}]
    ...
    async def test_button_flow_match_selection(self):
        ...
        with patch.object(wa, 'send_message') as mock_send_text, \
             patch('services.orchestrator.generate_betting_briefing', new_callable=AsyncMock) as mock_orchestrator, \
             patch('core.generate_bets.generate_bet_recommendations', new_callable=AsyncMock) as mock_bets:
            ...
            message_obj = {
                "interactive": {
                    "type": "list_reply",
                    "list_reply": {
                        "id": "analyze_argentina_vs_brazil"
                    }
                }
            }
...
class TestMultiAgentExecution:
    ...
    async def test_all_agents_contribute_to_god_view(self):
        ...
        with patch('core.initializer.llm.query_llm') as mock_llm:
            ...
            with patch('agents.tactics.api.sportmonks.fetch_team_stats') as mock_stats:
                ...
                with patch('agents.market.api.the_odds_api.fetch_latest_odds') as mock_odds:
                    mock_odds.return_value = []
    ...
    async def test_parallel_execution_speed(self):
        ...
        with patch('agents.logistics.logistics.LogisticsAgent.analyze', side_effect=slow_agent_mock), \
             patch('agents.tactics.tactics.TacticsAgent.analyze', side_effect=slow_agent_mock), \
             patch('agents.market.market.MarketAgent.analyze', side_effect=slow_agent_mock), \
             patch('agents.narrative.narrative.NarrativeAgent.analyze', side_effect=slow_agent_mock):
...
class TestErrorHandling:
    ...
    async def test_single_agent_failure_doesnt_crash_system(self):
        ...
        with patch('agents.logistics.logistics.LogisticsAgent.analyze') as mock_log:
            ...
            with patch('agents.tactics.tactics.TacticsAgent.analyze', new_callable=AsyncMock) as mock_tac:
                mock_tac.return_value = {"branch": "tactics", "team_a_xg": 2.0, "team_b_xg": 1.0}
                
                with patch('agents.market.market.MarketAgent.analyze', new_callable=AsyncMock) as mock_mkt:
                    mock_mkt.return_value = {"branch": "market_sniper", "best_odds": {}}
                    
                    with patch('agents.narrative.narrative.NarrativeAgent.analyze', new_callable=AsyncMock) as mock_nar:
                        mock_nar.return_value = {"branch": "narrative", "score": 5.0, "adjustment": 0}
                        
                        # Should NOT crash
                        god_view = await generate_betting_briefing(match_info)
                        
                        # Logistics should have fallback data
                        assert god_view['logistics']['fatigue_score'] == 5  # Default fallback
        ...
        test_list = {
            ...
            "action": {
                ...
                "sections": [{
                    "title": "Upcoming",
                    "rows": [
                        {"id": "match_1", "title": "Argentina vs Brazil", "description": "June 15"}
                    ]
                }]
            }
        }
