...
class Gatekeeper:
    ...
    async def _classify_async(message_body):
        ...
        if any(
            w in msg
            for w in [" vs ", " v ", "against", "odds", "bet", "staking", "budget"]
        ) or msg in ["analyze", "analyse", "run it", "do it"]:
        ...
        if (
            ...
            or msg.startswith("analyse")
            or msg.startswith("predict")
        ):
            extracted = await Gatekeeper._extract_match_details(message_body)
            return "BETTING", extracted
        ...
        try:
            category = await query_llm(
                GATEKEEPER_INTENT_PROMPT,
                f"User Input: {message_body}",
    ...
    async def _extract_match_details(message_body):
        ...
        budget_match = re.search(r"\$(\d+)", message_body) or re.search(
            r"(\d+)\s*(?:dollars|bucks|budget)", message_body.lower()
        )
        if budget_match:
        ...
        try:
            response = await query_llm(
                TEAM_EXTRACTION_PROMPT, message_body, json_mode=True, temperature=0.3
            )
            import json
