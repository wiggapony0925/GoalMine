    ...
    for outcome_key, true_prob, display_name in mapping:
        ...
        if current_odds_data:
            ...
            if isinstance(current_odds_data, dict):
                decimal_odds = float(
                    current_odds_data.get("odds", 0)
                    or current_odds_data.get("price", 0)
                )
                platform = current_odds_data.get(
                    "platform", "Market Consensus"
                ) or current_odds_data.get("book", "Unknown")
            else:
                decimal_odds = 0
                platform = "Unknown"
