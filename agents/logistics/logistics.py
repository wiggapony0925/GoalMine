...
class LogisticsAgent:
    ...
    async def analyze(self, prev_venue_name, next_venue_name, days_rest=4):
        ...
        raw_response = await query_llm(
            LOGISTICS_PROMPT, user_prompt, config_key="logistics"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to parse Logistics LLM response: {e}. Raw: {raw_response}"
            )
        ...
        return {
            ...
            "summary": analysis_json.get(
                "analysis_summary", analysis_json.get("analysis", "N/A")
            ),
